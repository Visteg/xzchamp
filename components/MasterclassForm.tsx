  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/submit-form', {
        ...
        body: JSON.stringify({ category: 'masterclass', formData }),
      })
        ...
        <div className="p-8 md:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3 pt-2">
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">
                  Я согласен с <a href="/privacy" target="_blank" className="text-[var(--neon-blue)] hover:underline">политикой конфиденциальности</a>
                </span>
              </label>
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-full text-white font-['Unbounded'] font-bold uppercase tracking-wide transition-all duration-300 mt-6 ${(isDisabled || isSubmitting) ? 'bg-gray-600 cursor-not-allowed opacity-50' : 'bg-[var(--neon-blue)] hover:shadow-[0_0_30px_var(--neon-blue)] hover:scale-105'}`}
            >
